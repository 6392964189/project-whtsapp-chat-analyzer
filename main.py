...
if  uploader is not None:
    ...
    if st.sidebar.button('show analysis'):
        ...
        if user_df.empty:
            ...
            fig, ax= plt.subplots()
            plt.bar(timeline_df['time'],timeline_df['message'],width=0.4)
            ax.set_xlim(-0.5, 5)
            plt.xlabel("month")
            ...
            fig, ax= plt.subplots()
            plt.scatter(daily_timeline_df['date'],daily_timeline_df['message'], color='green')
            plt.xlabel("days")
            plt.ylabel("frequency of message")
            plt.xticks(rotation=45)
            st.pyplot(fig)
            ...
            # st.title('Wordcloud')
            # df_wc= ana.create_wordcloud(selected_user, df)
            # fig, ax= plt.subplots()
            # ax.imshow(df_wc)
            # plt.axis("off")
            # st.pyplot(fig)
